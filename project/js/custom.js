...
function set_loading_pos(final){
	
	html_loading=$('<div style="height: 100%;background-color: rgba(0,0,0,0.5);position: fixed;top: 0px;left: 0px;right: 0px;z-index: 1050;" id="background_loading"></div>');
	if (final==0){
		x = $(window).width()/2;
		y = $(window).height()/2-20;
		$('#loading').css({'top': y});
		$('#loading').css({'left': x});
		if ($("#background_loading").length > 0){
		  $('#background_loading').remove();
		}
		$('#loading').after(html_loading);
	} else {
		x=-2000,
		y=-2000;
		$('#loading').css({'top': y});
