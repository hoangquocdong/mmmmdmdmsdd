...
$userid = (int)isset($_REQUEST['userid'])? $_REQUEST['userid'] : 0;
  ...
  CONNECT_DB();
	...
	*/
	...
	$returnarray = array(
		'status' => 500,
		'content'=>	'Delete investor info in db fail!'
	);

    $sql = 'DELETE FROM `investor` WHERE `id_investor`='.$id;

	$result = mysql_query($sql) or die(json_encode($returnarray));

	$returnarray = array(
		'status' => 200,
		'content'=>	'Delete investor info succesfully!'
	);

    $actioncode = 7; //delete investor profile
    $moredetail = 'delete investor : '.$id;
