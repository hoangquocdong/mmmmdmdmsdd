  ...
  CONNECT_DB();
	...
	function returnresult($object, $getmoresubinfo, $getallinvestor){
		...
		foreach ($object as $key => $value) {
			...
			foreach ($value['data'] as $keysub => $valuesub) {

				/*
				*	Lấy thông tin investor
				*/

				//echo '<pre>'; print_r($valuesub); exit();
				...
				if (isset($getmoresubinfo[$keys])){
					...
					if (isset($getallinvestor[$id_investor])){
						$valuesub ->$inskey = $getallinvestor[$id_investor];
					} else {$valuesub ->$inskey ='';}
				...
				*/
				...
				$hskey = 'hoso';
				$valuesub ->$hskey = getdocs_in_a_sub($ids);
