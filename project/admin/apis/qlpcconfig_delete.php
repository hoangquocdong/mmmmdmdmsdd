...
$userid = (int)isset($_REQUEST['userid'])? $_REQUEST['userid'] : 0;
  ...
  CONNECT_DB();
	...
	*/
	...
	$returnarray = array(
		'status' => 500,
		'content'=>	'Delete pc info in db fail!'
	);

    $sql = 'DELETE FROM `power_company` WHERE `id_pwc`='.$id;

	$result = mysql_query($sql) or die(json_encode($returnarray));

	$returnarray = array(
		'status' => 200,
		'content'=>	'Delete pc info succesfully!'
	);

    $actioncode = 4; //delete pc profile
    $moredetail = 'delete pc : '.$id;
